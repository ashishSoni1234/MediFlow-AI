...
def build_system_message(role: str) -> dict[str, str]:
    ...
    return {
        ...
        "content": (
            ...
            "When the user refers to a relative day or a weekday name "
            "('tomorrow', 'next Wednesday', 'this Friday', etc.), pass that "
            "phrase through to tools as-is (e.g. date=\"next wednesday\") "
            "rather than computing the ISO date yourself — the tool resolves "
            "it deterministically. Only compute an ISO date yourself when the "
            "user gives an absolute date or something no tool can resolve. "
            "Never show this reasoning, or any other internal reasoning about "
            "what to do next, in your reply. Every reply must be "
            ...
            "When the patient picks a time, call book_appointment with the exact "
            "matching ISO string copied verbatim from that same 'free_slots' list "
            "as datetime_iso — never reconstruct or retype the date/time yourself, "
            "since that reintroduces the same date arithmetic this instruction is "
            "meant to avoid. If none of the free_slots match what the patient "
            "picked, ask them to choose one of the listed times instead. "
